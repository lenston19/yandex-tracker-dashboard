...
- Недельный виджет: норма часов теперь рассчитывается с учётом выходных и праздничных дней по производственному календарю
- Исправлен брейкпоинт разделителя месяцев
::
::card{icon="i-lucide-mouse-pointer" color="info"}
- Недельный виджет: добавлен hover-эффект при наведении на день с записями
::
::
...
::card-group
::card{icon="i-lucide-calendar-days" color="primary"}
- Недельный виджет: под названием каждого дня теперь отображается короткая дата («31 мар», «1 апр»)
- Если неделя пересекает границу месяца — первый день нового месяца визуально выделен: цветной разделитель слева и бейдж с названием месяца
::
::card{icon="i-lucide-rocket" color="info"}
- Добавлено автоматическое тестирование при открытии PR в `dev` и `master`
::
::
...
::card-group
::card{icon="i-lucide-bug" color="warning"}
- Сдвиг дней недели при UTC+6 и выше
- Heatmap не показывал текущую неделю
::
::card{icon="i-lucide-refresh-ccw" color="info"}
- Компонент «График активности» переписан с нуля — убрана сторонняя библиотека (ApexCharts)
- Ключи задач во всех таблицах теперь являются кликабельными ссылками на Yandex Tracker
::
::
...
::card-group
::card{icon="i-lucide-bug" color="warning"}
- Исправлен `RangeError: Invalid time value` при форматировании дат — возникал у пользователей со старым форматом cookie временной зоны
- Добавлена автоматическая миграция: при загрузке приложения устаревший формат `timeZone` исправляется и сохраняется корректно
::
::card{icon="i-lucide-rocket" color="info"}
- Настроен автоматический CI/CD релиз при merge в `master`
::
::
...
::card-group
::card{icon="i-lucide-activity" color="primary"}
- Виджет «График активности» — Heatmap за выбранный период (3, 6, 9 месяцев или 1 год)
::
::card{icon="i-lucide-calendar-check" color="primary"}
- Российский производственный календарь через `isdayoff.ru` — праздники и переносы учитываются в недельном виджете и сводке месяца
- Праздничные дни в недельном виджете отмечаются как «Выходной»
::
::card{icon="i-lucide-clock" color="info"}
- Даты и время теперь отображаются корректно в выбранной пользователем временной зоне
- Внутренние улучшения: миграция на `date-fns`, унификация форматирования дат, улучшена архитектура модулей
- Страница настроек переработана
- Тултип у подсказки «нужно X ч/день» в сводке месяца с объяснением расчёта
::
::card{icon="i-lucide-bug" color="warning"}
- Исправлена утечка памяти при загрузке аватара пользователя
- Исправлен баг с состоянием загрузки на главной странице
- Исправлена ошибка прогресс-бара при переработках сверх нормы
::
::
